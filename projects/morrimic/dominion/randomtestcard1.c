...
int main() {
	...
	*/
	...
	for (int i = 0; i < NUMTESTS; i++) {
		...
		handpos = rand() % 5;
		gameState.hand[currentPlayer][handpos] = village;
		...
		// Verify hand count unchanged (+1 card, -1 discard)
		...
		// Verify village was discarded.
		else if (gameState.discard[currentPlayer][gameState.discardCount[currentPlayer] - 1] != village) {
			if (NOISYTEST) printf("Incorrect discard top card.\n");
			numFails++;
		}
		// Verify actions increased by 1 (+2 from village, -1 from playing village).
